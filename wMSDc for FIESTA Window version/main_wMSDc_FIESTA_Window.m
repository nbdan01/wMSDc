...
% if you want to take dt from the Metadata, let "dt = "Metadata"", else write
% the numerical value in seconds (s)

dt = "Metadata";
% Taking dt from the Metadata might induce some uncertainties if the number
% between each consecutive frame is not constant. To analyse the
% trajectories using wMSDc, this number will be calculated as equal to the
% median of the distribution and will be set constant.
...
%% Advanced Settings: 
    ...
    %% OPTION Fixed window width (1 == ON; 0 == OFF)
...
%% START
%% Import data
path = 'E:\Script wMSDc for Matlab\Data\Motor proteins Fiesta\';
file = 'Che11_Velocity_Data.xlsx';
...
% Check File format (file coming from Fiesta)
if exist('TrackData','var') ~= 1 % This is a mat file coming from FIESTA
    msgfig = msgbox('This is not an xlsx file.','Error','modal');
else
    load([path,file])
    ...
    names =  fieldnames(TrackData(1));
    
    % Loop over all the datasets in the file
    for ik = 1:size(names,1)

        value = struct2cell(TrackData);
        value_for_each_molecule = value{ik};

        
        
        % Loop over all the molecules in the file
        for ik_part = ik_part_start:size(value_for_each_molecule,2)

            [ik_part ik size(value_for_each_molecule,2)]

            % Get the data for this trajectory
            % Store X and Y values
            % Create a subFolder with the name of each
            % different data subFolders
            % Create the folder results if not existing
            Foldersave = [Foldersaveall,names{ik},'\'];
            ...
            % Get the data for this trajectory
            data_part = cell2mat(value_for_each_molecule(ik_part));
            % Number of Frames
            Frame_number = data_part(:,1);
            % Store Time
            Time =  data_part(:,2);
            ...
            % missing values
            TIMEh = Frame_number-min(Frame_number)+1;
            TIME = [min(Frame_number)-min(Frame_number)+1:max(Frame_number)-min(Frame_number)];

          
            % Convert in second - dt

            if class(dt) == 'double'
                
            else
                % Calculate the minimal integration time - dt
                % (s) (dt can be different if extracted from
                % the metadata. Here we take the MEDIAN!!!)
            ...
            XC = interp1(TIMEh,XC,TIME)*pxl_to_um/1000; % X and Y converted in um
            YC = interp1(TIMEh,YC,TIME)*pxl_to_um/1000;
            TIME = TIME';
            XC = XC';
            YC = YC';
            ...
            % Calculate MSD
            numberoflagsmax = 600;
            [LagTtt,MSD_meantt,MSD_stdtt,yfittt,ptt] = MSD(XC,TIME*dt,min((size(TIME,1)),numberoflagsmax),dt);
...
%                             alpha_averaged_velocity = NEW_MATRIX(2,ii_noise);
            ...
            if size(XC,1)<=Window_width
                ...
                name = ['Track' num2str(ik_part,'%.0f')];
                Data.(name) = Class_column;
            ...
            if ik == size(names,1) && ik_part == size(value_for_each_molecule,2)
                
                msgfig = msgbox('Operation was completed successfully!','Success','modal');
            end
        end
        save([path,'Results\','All_track_Resalpha.mat'],'Dataw')
end
